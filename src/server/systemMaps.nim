...
proc spawnMaps(gserver: Gserver) =
...
proc connectEvents(gserver: Gserver) =
  ...
  gserver.reg.connect(EvPlayerDisconnected,
    proc (ev: EvPlayerDisconnected) =
      ...
      gserver.removePlayerFromALlMaps(ev.entPlayer)
  )

proc newSystemMaps*(gserver: Gserver): SystemMaps =
  gserver.loadMaps()
  gserver.spawnMaps()
  gserver.connectEvents()
